/**
 * @file basiccalc.h
 * Basic calculator (template class).
 *
...
#include "util/baroneadesiwhaley.h"
#include "util/bjerksundstensland02.h"
#include "util/bjerksundstensland93.h"
...
class BasicCalculator : public AbstractExpectedValueCalculator<C, VI>
{
    using _Myt = BasicCalculator<C, VI>;
...
public:
     ...
     */
    BasicCalculator( double underlying, const table_model_type *chains, item_model_type *results ) :
...
protected:
    ...
    virtual AbstractOptionPricing *createPricingMethod( double S, double r, double b, double sigma, double T, bool european = false ) const override
    {
        ...
        return new pricing_method_type( S, r, b, sigma, T );
    }

private:

    // not implemented
    BasicCalculator( const _Myt& ) = delete;
